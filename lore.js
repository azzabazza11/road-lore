...
function buildLorePrompt({ lat, lng, avoid = [], expand = null, interests, length } = {}) {
  ...
  if (expand && expand.text) {
    ...
    return [
      ...
      where,
      'They tapped More and just heard this short piece titled "' + title + '":',
      String(expand.text),
      'This is a FOLLOW-UP of that same tale — not a new nearby story.',
      'Use web search to continue THIS SAME topic with ' + spec.expand + ': what happened next, more colour,',
      'or a surprising fact they have not heard. Stay on the same subject and lean into the traveller’s interests when relevant:',
      ids.map(id => INTERESTS[id].label.toLowerCase()).join(', ') + '.',
