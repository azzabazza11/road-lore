...
- **Speech synthesis / "Voice error" is expected in the VM.** The app narrates via `speechSynthesis`; a headless VM Chrome usually has no voices/audio, so the voice status may show "Voice error". This is an environment limitation, not an app bug — the visual lookup + log flow still works.
